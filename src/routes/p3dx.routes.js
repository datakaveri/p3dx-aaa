...
router.post('/workloads/preview-contract', verifyJWT, requireRole('user'), async (req, res, next) => {
  try {
    ...
    // datasets is an array of {datasetId, datasetName} pairs — any number
    // >= 1 (dataset picker now allows multi-select; see WorkloadForm.jsx).
    // datasetId is each dataset's real item_id (see formSubmissions.service.js's
    // listDatasetNames); datasetName falls back to the id for older/other
    // callers that never sent a separate name.
    const datasets = Array.isArray(req.body?.datasets)
      ? req.body.datasets.map(d => ({
          datasetId: d.datasetId || d.dataset,
          datasetName: d.datasetName || d.datasetId || d.dataset,
        }))
      : [];
    const technique = req.body?.technique;
    const applicationId = req.body?.applicationId || req.body?.application || 'unselected-application';
    // infraIds is the InfraCat selection (SMPC only) — exactly 2 required.
    const infraIds = Array.isArray(req.body?.infraIds) ? req.body.infraIds : (Array.isArray(req.body?.infra_ids) ? req.body.infra_ids : []);

    if (datasets.length === 0 || datasets.some(d => !d.datasetId)) {
      return res.status(400).json({ status: 'FAILED', error: 'MISSING_DATASET_IDS' });
    }
    if (!['FL', 'TEE', 'SMPC'].includes(technique)) {
      return res.status(400).json({ status: 'FAILED', error: 'INVALID_TECHNIQUE' });
    }
    // Second line of defense alongside gov_layer's own check and the UI's cap.
    if (technique === 'SMPC' && infraIds.length !== 2) {
      return res.status(400).json({ status: 'FAILED', error: 'INFRA_COUNT_MUST_BE_2' });
    }
